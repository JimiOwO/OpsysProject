...
class proc():
    def __init__(self,rand,id,type):
        ...
        self.bursts = m.ceil(rand.drand48())
        self.id = id
        self.timing =[]
        self.rand=rand
    ...
    def generate(self):
        if self.bound_type == "CPU":
            for _ in range(self.bursts-1):
                ...
                io  = m.ceil(self.rand.next_exp())
                io *= 10
                cpu *=4
                io  /=8
                self.timing.append((cpu,io))
            cpu = m.ceil(self.rand.next_exp()) *4
            self.timing.append((cpu,0))
    ...
    def print_proc(self):
        ...
        print(f"{self.bound_type}-bound process {self.id}: arrival time {self.arrival}; {self.bursts} CPU bursts:")
